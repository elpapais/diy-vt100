...
vt100_buffer_putchar()
{
	...
	{
		vt100_buffer[vt100_cursor.row][vt100_cursor.col].prop |= VT100_CHAR_PROP_DATA_BOLD;
	}
	
	if(setting_read(SETTING__ATTR_UNDERLINE))
	{
		vt100_buffer[vt100_cursor.row][vt100_cursor.col].prop |= VT100_CHAR_PROP_DATA_UNDERLINE;
	}
	
	if(setting_read(SETTING__ATTR_INVERSE))
	{
		vt100_buffer[vt100_cursor.row][vt100_cursor.col].prop |= VT100_CHAR_PROP_DATA_INVERSE;
	}
...
vt100_buffer_shiftup()
{
	...
	vt100_buffer[0][0].prop |= VT100_CHAR_PROP_ROW_TOUCH;
	...
	switch(param.data[0])
	{
		...
		case 2:
			...
			for(i = 0; i < VT100_HEIGHT; i++)
			{
				...
				vt100_buffer[i][0].prop &= ~VT100_CHAR_PROP_ROW_DOUBLE_WIDTH;
				vt100_buffer[i][0].prop |= VT100_CHAR_PROP_ROW_TOUCH;
